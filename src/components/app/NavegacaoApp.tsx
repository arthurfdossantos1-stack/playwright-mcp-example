  ...
  },
  {
    ...
    itens: [
      ...
      },
      {
        ...
        icone: (
          <path
            d="m3 3 18 9-18 9 4.5-9L3 3Zm4.5 9H12"
            fill="none"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
...
export function NavegacaoApp({
  ...
  function badgeDoItem(href: string): number {
  ...
  const conteudoNav = (
    <>
      <nav className="flex-1 space-y-5 overflow-y-auto px-3 pb-3" aria-label="Navegação do app">
        {GRUPOS.map((grupo) => (
          <div key={grupo.titulo}>
            <p className="mb-1.5 px-3 text-[10.5px] font-bold uppercase tracking-wider text-slate-400">
              {grupo.titulo}
            </p>
      ...
      <div className="mt-auto shrink-0 border-t border-slate-200 p-3">
        ...
        >
          <svg viewBox="0 0 24 24" className="h-[18px] w-[18px] shrink-0" fill="none" stroke="currentColor" strokeWidth="1.7">
            <path
        ...
        <button
          type="button"
          onClick={sair}
          className="mt-1 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-100 hover:text-slate-900 lg:py-2"
        >
    ...
    <>
      ...
      <div className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur lg:hidden">
        ...
        <button
          ...
          className="relative grid h-10 w-10 place-items-center rounded-lg border border-slate-200 text-slate-700 active:bg-slate-50"
      ...
      {aberto && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          />
          <aside className="absolute left-0 top-0 flex h-full w-[19rem] max-w-[85vw] flex-col bg-white pt-4 shadow-xl">
